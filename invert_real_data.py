...
# Read in the electrode locations and measured data from the res2dinv data file
file_path = "data/Project4_Wenner_1.dat"
survey, topo_2d, dobs, error = res2dinv_to_real_data(file_path)

# set data expected error and put it in a simpeg data item
data_dc = data.Data(survey, dobs=dobs, relative_error=std)

# have a look at the data
plot_2_pseudosections(survey, dobs)
...
# number of active cells, then mapping
n_active = np.sum(active_cells)
log_resistivity_map = maps.InjectActiveCells(
    mesh, active_cells, np.log(1e8)  # air resistivity
...
# define starting model from average of apparent resistivities of real data
starting_log_resistivity_model = np.log(
    build_starting_model(survey, dobs, active_cells)
)
